...
class GYImageLoader : ImageLoader {
  /**
   * 根据自身photo数据加载图片.可以使用其它图片加载框架.
   */
  override fun load(view: ImageView, data: Photo, viewHolder: RecyclerView.ViewHolder) {
    val it = (data as? GYPhoto?)?.url ?: return
...
//      viewHolder.itemView.findViewById<PlayerControlView>(R.id.playerControlView)
...
//    val url = (data as? GYPhoto?)?.url ?: return
...
//          ImageSource.uri(Uri.fromFile(it)).also { source -> subsamplingCache.put(url, source) })
//      }
